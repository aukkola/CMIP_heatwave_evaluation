...
DIR="/g/data/w35/amu561/CMIP5_heatwaves/CMIP5_Data"
...
search_criteria="--local $dataset --experiment historical --experiment rcp85 \
                 --variable tasmax --variable tasmin \
                 --mip fx --mip day"
    ...
    do
        ...
        do
            ...
            do
              ...
              if [ $year_start -gt $start_yr ] 
              then
                echo "ERROR: cannot process ${E}, $V, ${M}, requested start year before dataset start"
                continue
              fi
              
              if [ $year_end -lt $end_yr ] 
              then
                echo "ERROR: cannot process ${E}, $V, ${M}, requested end year after dataset end"
                continue
              fi            
              
              
              #Select years if start year is greater or end year less than dataset years
              if [ $start_yr -gt $year_start -o $end_yr -lt $year_end ]
              then
                
                yrsel_temp="${processed_path}/Yr_sel_${var_short}_${M}_${E}_${ens}.nc"
                
                cdo selyear,$start_yr/$end_yr $in_file $yrsel_temp
                
                  
                #Replace years in in_file                
                in_file=${out_merged%".nc"}"_"${start_yr}"01-"${end_yr}"12.nc"

                
                #Replace merged output file with corrected file name
                mv $yrsel_temp $in_file
              ...
              ###--- Daily tasmin ---###
              elif [[ $V =~ "day/tasmin" ]]; then

                  #Create output file
                  out_file="${processed_file}_deg_C_${year_start}_${year_end}_${E}.nc"

                  #Convert Kelvin to Celsius (change unit, average to monthly, select years and convert to C)
                  cdo expr,'tasmin=tasmin-273.15' -setunit,'degrees C' $in_file $out_file
